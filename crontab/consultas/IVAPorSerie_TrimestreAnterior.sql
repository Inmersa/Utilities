...
DATE_FORMAT(IF( DATE_FORMAT(NOW(),'%m') >= 1 AND DATE_FORMAT(NOW(),'%m') <= 3,
CONCAT((DATE_FORMAT(NOW(),'%Y')-1),'-10-00 00:00:00'),CONCAT(DATE_FORMAT(NOW(),'%Y'),'-',FLOOR((DATE_FORMAT(NOW(),'%m')-1)/3) * 3 - 2,'-00 00:00:00') ),'%Y-%m-%d %H:%i:%s')
AND 
DATE_FORMAT(IF( DATE_FORMAT(NOW(),'%m') >= 1 AND DATE_FORMAT(NOW(),'%m') <= 3,
CONCAT((DATE_FORMAT(NOW(),'%Y')),'-00-00'),CONCAT(DATE_FORMAT(NOW(),'%Y'),'-',FLOOR((DATE_FORMAT(NOW(),'%m')-1)/3) * 3 ,'-31 23:59:59') ),'%Y-%m-%d %H:%i:%s')

#FACTURAS DE ABONO - Biomundo
# SERIE 0 A 3 = ventas, abonos=6, UE_Abonos=7
#SUBSTRING(F.referencia,4) as B_NUM_factura (para facturas)
#SUBSTRING(F.referencia,9) as B_NUM_factura (para abonos)
#SUBSTRING(F.referencia,12) as B_NUM_factura (para UE_abonos)
SELECT
IF(F.id_serie=0,1,F.id_serie) as A_tipo_documento,
SUBSTRING(F.referencia,9) as B_NUM_factura,
...
ROUND(SUM(IF(IV.id_iva=1,-1*IV.monto,0)),2) as S_importe_neto_1,
ROUND(SUM(IF(IV.id_iva=7,-1*IV.monto,0)),2) as T_importe_neto_2,
ROUND(SUM(IF(IV.id_iva=8,-1*IV.monto,0)),2) as U_importe_neto_3,
ROUND(((F.descuento/F.total_bruto)*100),2) as V_porcentaje_dto_1,
'' as W_porc_dto_2, ' ' as X_porc_dto_3,
ROUND((-1*F.descuento),2) as Y_importe_dto_1, ' ' as Z_importe_dto_2, ' ' as AA_importe_dto_3,
...
ROUND(SUM(IF(IV.id_iva=1,-1*IV.monto,0)),2) as AT_BaseImp_1,
ROUND(SUM(IF(IV.id_iva=7,-1*IV.monto,0)),2) as AU_BaseImp_2,
ROUND(SUM(IF(IV.id_iva=8,-1*IV.monto,0)),2) as AV_BaseImp_3,
...
ROUND(SUM(IF(IV.id_iva=1,-1*IV.recargo_iva,0)),2) as AZ_ImporteIVA_1,
ROUND(SUM(IF(IV.id_iva=7,-1*IV.recargo_iva,0)),2) as BA_ImporteIVA_2,
ROUND(SUM(IF(IV.id_iva=8,-1*IV.recargo_iva,0)),2) as BB_ImporteIVA_3,
...
ROUND(SUM(IF(IV.id_iva=1,-1*IV.recargo_equivalente,0)),2) as BF_importeRE_1,
ROUND(SUM(IF(IV.id_iva=7,-1*IV.recargo_equivalente,0)),2) as BG_importeRE_2,
ROUND(SUM(IF(IV.id_iva=8,-1*IV.recargo_equivalente,0)),2) as BH_importeRE_3,
IF(F.retenido>0,F.retenido/F.base_imponible,0) as BI_porc_retencion, (-1*F.retenido) as BJ_imp_retencion,
ROUND((-1*F.monto_total),2) as BK_total, F.id_forma_pago as BL_id_forma_pago, '0' as BM_portes, 'Pagados' as BN_texto_portes, F.referencia_externa as BO_observ_linea_1, ' ' as BP_observ_linea_2, ' ' as QB_obra, ' ' as BR_remitido_por, ' ' as BS_embalado_por, ' ' as BT_att,
...
AND F.id_serie>=6
AND F.id_serie<=6
AND F.fecha >= DATE_SUB(CURDATE(),INTERVAL 1 DAY)

#FACTURAS DE ABONO - planet
# SERIE 4 = ventas (convertir a 1 para Factusol), Serie S=3 (una fra en 2012), Abonos=7
#SUBSTRING(F.referencia,3) as B_NUM_factura (para facturas)
#SUBSTRING(F.referencia,9) as B_NUM_factura (para abonos-SERIE 7)
USE planet_gestion;
SELECT
IF(F.id_serie=4,1,F.id_serie) as A_tipo_documento, 
...
IF(F.retenido>0,F.retenido/F.base_imponible,0) as BI_porc_retencion, ROUND(F.retenido,2) as BJ_imp_retencion, 

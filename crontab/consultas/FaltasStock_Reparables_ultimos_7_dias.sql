...
P.referencia as "Pedido",DATE_FORMAT(P.fecha,'%Y-%m-%d') as "Fecha",C.razon_social as "Cliente",
D.nombre as "Nombre De Direccion", A.id_articulo as "ID Art" ,A.nombre as "Articulo" ,M.nombre as "Marca",
ROUND(Ld.cantidad) as "Falta", ROUND(SUM(L.cantidad)) as "Existencias"
...
WHERE P.fecha >= DATE_SUB(NOW(),INTERVAL 7 DAY) AND P.residual > 0 AND L.cantidad != 0 AND PF.id_factura IS NULL AND P.preparado = 0
